...
void loop() {
  ...
  if (button1State == LOW)    // If button1Pin is low jump to cases on switch 2
  {
    if (button2State == LOW) // Checking switch 2 cases to perform stream of pulses
    {
      ...
      delay(50); // Passing the delay for the HIGH red light of 50 microseconds
      ...
      for (int i = 0; i < c; i++) //For loop for the Normal wave mode
      {
        ...
        delay(a + (i * 50)); // Incremental delay of 50 microseconds with each cycle
        digitalWrite(led1Pin, LOW);
        delay(b); // delay of b microseconds between consecutive pulses
      }
      delay(d); // Delay of d microseconds after completion of one cycle of C pulses
    }
    ...
    else // run with a modified waveform cycle (Inverted waveform)
    {
      ...
      {
        digitalWrite(led1Pin, HIGH);  
        delay(a + ((i - 1) * 50)); // Decremental delay of 50 microseconds with each cycle
        digitalWrite(led1Pin, LOW);
        delay(b); // delay of b microseconds between consecutive pulses
      }
      delay(d); // Delay of d microseconds after completion of one cycle of C pulses
